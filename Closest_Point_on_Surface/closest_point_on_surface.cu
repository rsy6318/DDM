...
/*__device__ void closestPointOnTriangle2(
    const float a,
    const float b,
    const float c,
    const float d,
...
)
{
    ...
    //printf("%d",m);
    torch::Tensor w1=torch::zeros({n},torch::CUDA(torch::kFloat));
    torch::Tensor w2=torch::zeros({n},torch::CUDA(torch::kFloat));
    torch::Tensor w3=torch::zeros({n},torch::CUDA(torch::kFloat));
    torch::Tensor indexes=torch::zeros({n},torch::CUDA(torch::kInt));
